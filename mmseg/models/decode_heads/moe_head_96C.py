...
class MOEHead(BaseDecodeHead):
    ...
    def __init__(self, feature_strides, prescale_mlp_dims=None, prescale_mlp_final_act=True,
                 afterscale_mlp_dims=[192, 96], afterscale_mlp_final_act=True, moe_mlp_dims=[192, 96], moe_conv_dims=None, activation='relu', use_linear_fuse=True,dim=96, **kwargs):
        ...
        self.dim=dim
        for i in range(4):
            self.swin_expand.append(swin_Expand(dim=int(96*2**i), dim_scale=2, norm_layer=nn.LayerNorm))
            # self.swin_expand.append(i_layer)
        # self.swin_expand=swin_Expand(dim=512, dim_scale=2, norm_layer=nn.LayerNorm)
        ...
        # self.up =up()
        # self.conv = nn.Conv2d(in_channels=96, out_channels=96, kernel_size=3, stride=1, padding=1)
    ...
    def forward(self, inputs, img_metas=None):
        ...
        x_stacked = torch.stack(x_scaled, dim=1)    #堆叠，准备乘权重
        x = torch.cat(x_scaled, dim=1)      #concat后准备做softmax

        if self.moe_conv is not None:
            for conv_layer in self.moe_conv:
                x = conv_layer(x)       #对concat后结果做3*3卷积

        if self.moe_mlp is not None:
            x = self.moe_mlp(x)     #对卷积后结果做mlp

        moe_weights = torch.softmax(x, dim=1)   #生成权重
        x = (x_stacked * moe_weights.unsqueeze(2)).sum(1)   #相乘，每个点和为1


        if self.afterscale_mlp_dims is not None:
            x = self.afterscale_mlp(x)      #对结果在做mlp

        if self.use_linear_fuse:
            x = self.linear_fuse(x)     #1*1卷积
...
if __name__ == '__main__':
    ...
    print(input_3.shape)
    print(input_4.shape)
    decode = MOEHead(prescale_mlp_dims=[96, 96],  # new
