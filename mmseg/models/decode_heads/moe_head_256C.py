...
class swin_Expand(nn.Module):      #new!!!!
    def __init__(self, dim=256, dim_scale=2, norm_layer=nn.LayerNorm):
        super().__init__()
        self.dim = dim
...
class swin_Expand_x4(nn.Module):      #new!!!!
    def __init__(self, dim=256, dim_scale=4, norm_layer=nn.LayerNorm):
        super().__init__()
        self.dim = dim
        self.dim_scale = dim_scale
...
class MOEHead(BaseDecodeHead):
    ...
    def __init__(self, feature_strides, prescale_mlp_dims=None, prescale_mlp_final_act=True,
        ...
        self.dim=dim
        for i in range(4):
            self.swin_expand.append(swin_Expand(dim=int(dim*2**i), dim_scale=2, norm_layer=nn.LayerNorm))
