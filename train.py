...
from training.validation import validation,validation_flare21
...
def train_net(net, args, ema_net=None, fold_idx=0):
    ...
    for epoch in range(args.epochs):
        ...
        # exp_scheduler = exp_lr_scheduler_with_warmup(optimizer, init_lr=args.base_lr, epoch=epoch, warmup_epoch=30, max_epoch=args.epochs)
        exp_scheduler = 1e-5
        ...
        if (epoch+1) % args.val_frequency == 0:
            ...
            dice_list_test, ASD_list_test, HD_list_test = validation_flare21(net_for_eval, testLoader, args)
...
def get_parser():
    ...
    parser.add_argument('--model', type=str, default='HCTNet', help='model name')
    parser.add_argument('--dimension', type=str, default='2d', help='2d model or 3d model')
    parser.add_argument('--pretrain', default=False, help='if use pretrained weight for init')
    # parser.add_argument('--pretrain', default=True,action='store_true', help='if use pretrained weight for init')
    # parser.add_argument('--pretrain', default=False,action='store_true', help='if use pretrained weight for init')
    ...
    # parser.add_argument('--unique_name', type=str, default='acdc_2dshufflemoe5.5epoch1000val5lr0.0001wd0.0005nopre', help='unique experiment name')
    parser.add_argument('--unique_name', type=str, default='acdc_HCTNet9.11',help='unique experiment name')
