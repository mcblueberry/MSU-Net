...
class CMRDataset(Dataset):
    def __init__(self, args, mode='train', k_fold=5, k=0, seed=0):
        ...
        if mode == 'train':
            root = "/opt/data/private/zjm/UTnetV2/dataset/flare21_train"
        else:
            root = "/opt/data/private/zjm/UTnetV2/dataset/flare21_test"
        ...
        test_name_list = img_name_list[0:70]
        ...
        for name in img_name_list:
            # for idx in [0, 1]:
            img_name = str(name) + '.nii.gz'
            lab_name = str(name) + '_gt.nii.gz'
        ...
        if self.mode == 'test':
            for i in range(len(self.img_list)):
                tmp_img = self.img_list[i]
                tmp_lab = self.lab_list[i]
    ...
    def __getitem__(self, idx):
        ...
        if self.mode == 'test':
            tensor_img, tensor_lab = tensor_img.unsqueeze(0), tensor_lab.unsqueeze(0)  ##改
...
#         assert mode in ['train', 'test']
#         if mode == 'train':
#             root = "/opt/data/private/zjm/UTnetV2_new/dataset/flare21_train"
#         else:
#             root = "/opt/data/private/zjm/UTnetV2_new/dataset/flare21_test"
#         with open(os.path.join(args.data_root, 'list', 'dataset.yaml'), 'r') as f:
#             img_name_list = yaml.load(f, Loader=yaml.SafeLoader)
#
#         random.Random(seed).shuffle(img_name_list)
